...
describe('side annotation content', () => {
  it('keeps old combined annotations readable', () => {
    expect(sideAnnotationContent(base)).toEqual({ code: 'A1', title: 'Aigua', meta: 'Km 10,2 · 15:30–19:30', description: 'Fruita · Aigua' });
  });
  ...
  it('uses separately authored fields and localized details', () => {
    ...
    expect(sideAnnotationContent(localizedAnnotation(annotation, 'en'))).toEqual({ code: 'B2', title: 'Oranges', meta: 'Km 33.1 · 17:50–1:10', description: 'Oranges' });
  });
});
